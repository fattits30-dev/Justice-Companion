...
fetch(BASE_URL)
  ...
  .then(response => {
    if (response.ok) {
      ...
      return response.json().then(err => {
        console.log(`   ⚠️  Registration response: ${response.status} - ${JSON.stringify(err)}`);
        return err;
      });
    }
  })
  .then(data => {
    ...
    console.log('      - Case types: Employment, Housing, Consumer, Debt, Small Claims');
    ...
    console.log('      - Configurable AI providers (HuggingFace, OpenAI, Anthropic, etc.)');
  ...
  .catch(error => {
    ...
    console.error('  - Check ports 5176 (frontend) and 8000 (backend) are available');
  });
