...
describe('RegistrationScreen', () => {
  ...
  test('calls window.justiceAPI.register with correct data', async () => {
    ...
    window.justiceAPI.register = mockRegister;
    ...
    fireEvent.change(screen.getByPlaceholderText(/enter your password/i), { target: { value: 'StrongPass123!' } });
    fireEvent.change(screen.getByPlaceholderText(/confirm your password/i), { target: { value: 'StrongPass123!' } });
    ...
    // Verify IPC call
    await waitFor(() => {
      expect(mockRegister).toHaveBeenCalledWith('testuser', 'test@example.com', 'StrongPass123!');
    });
  });
  ...
  test('shows loading state while registering', async () => {
    const mockRegister = vi.fn().mockImplementation(() =>
      new Promise(resolve => setTimeout(() => resolve({ success: true, data: { user: {} } }), 100))
    );
    window.justiceAPI.register = mockRegister;
    ...
    fireEvent.click(screen.getByRole('button', { name: /sign up|register|create account/i }));

    // Check loading state
    await waitFor(() => {
      expect(screen.getByText(/creating account|registering|loading/i)).toBeInTheDocument();
    });
  });
  ...
  test('shows error message when registration fails', async () => {
    ...
    fireEvent.change(screen.getByPlaceholderText(/enter your username/i), { target: { value: 'existinguser' } });
    fireEvent.change(screen.getByPlaceholderText(/enter your email/i), { target: { value: 'test@example.com' } });
    fireEvent.change(screen.getByPlaceholderText(/enter your password/i), { target: { value: 'StrongPass123!' } });
    fireEvent.change(screen.getByPlaceholderText(/confirm your password/i), { target: { value: 'StrongPass123!' } });
    fireEvent.click(screen.getByLabelText(/i agree to the terms/i));
  ...
  test('shows link to go back to login screen', () => {
    ...
    const loginLink = screen.getByRole('button', { name: /already have an account|sign in|login/i });
