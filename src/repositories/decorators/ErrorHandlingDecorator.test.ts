...
class MockRepository {
  ...
  async delete(id: number) {
    ...
    if (id === 777) {
      throw new Error('FOREIGN KEY constraint failed: referenced by other records');
    }
    return true;
  }
}
...
describe('ErrorHandlingDecorator', () => {
  ...
  describe('NotFoundError handling', () => {
    ...
    it('should throw NotFoundError when update returns false', async () => {
      await expect(decorator.update(999, { name: 'Test' })).rejects.toThrow(NotFoundError);
    });
  ...
  describe('Generic errors', () => {
    it('should wrap unknown errors as RepositoryError', async () => {
      vi.spyOn(mockRepository, 'findById').mockRejectedValueOnce(new Error('Unknown error'));
  ...
  describe('Error context', () => {
    ...
    it('should sanitize sensitive fields', async () => {
      ...
      try {
        ...
        if (error instanceof RepositoryError) {
          expect(error.context?.input.password).toBe('[REDACTED]');
          expect(error.context?.input.apiKey).toBe('[REDACTED]');
          expect(error.context?.input.token).toBe('[REDACTED]');
          expect(error.context?.input.name).toBe('Test');
        }
      }
    });
  });
  ...
  describe('Options', () => {
    it('should include stack trace when enabled', async () => {
      ...
      vi.spyOn(mockRepository, 'findById').mockRejectedValueOnce(new Error('Test error'));
