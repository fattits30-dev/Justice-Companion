...
const EXCLUDE_PATTERNS = [".test.tsx", ".example.tsx", "test-utils.tsx"];
...
function analyzeComponent(filePath: string): ComponentAnalysis | null {
  ...
  if (
    HIGH_PRIORITY_PATTERNS.some((pattern) => componentName.includes(pattern))
  ) {
    priority = "high";
...
function analyzeReactComponents(): void {
  logger.info("Analyzing React components for optimization opportunities...\n");

  const srcDir = join(process.cwd(), "src");
  const tsxFiles = findTsxFiles(srcDir);

  logger.info(`Found ${tsxFiles.length} component files\n`);
  ...
  const withoutUseCallback = analyses.filter(
    (a) => a.hasInlineHandlers && !a.hasUseCallback
  );

  logger.info("SUMMARY\n");
  logger.info("=".repeat(80));
  logger.info(`Total Components: ${analyses.length}`);
  logger.info(
    "ReactAnalysis",
    `Without React.memo(): ${withoutMemo.length} (${Math.round((withoutMemo.length / analyses.length) * 100)}%)`
  );
  logger.info(
    "ReactAnalysis",
    `With inline handlers, no useCallback: ${withoutUseCallback.length}`
  );
  logger.info("=".repeat(80));
  logger.info("");
  ...
  logger.info(
    "ReactAnalysis",
    `HIGH PRIORITY (${highPriority.length} components)\n`
  );
  logger.info("These should be optimized first:\n");

  highPriority.forEach((a) => {
    logger.info(`  ${a.componentName}`);
    logger.info(`     File: ${a.file.replace(srcDir, "src")}`);
    logger.info(`     Reason: ${a.reason}`);
    logger.info(
      "ReactAnalysis",
      `     Memo: ${a.hasMemo ? "Yes" : "No"} | useCallback: ${a.hasUseCallback ? "Yes" : "No"} | LOC: ${a.linesOfCode}`
    );
    logger.info("");
  });
  ...
  logger.info(
    "ReactAnalysis",
    `MEDIUM PRIORITY (${mediumPriority.length} components)\n`
  );
  logger.info("Should be optimized after high priority:\n");
  ...
  mediumPriority.slice(0, 10).forEach((a) => {
    ...
    logger.info(
      "ReactAnalysis",
      `     Memo: ${a.hasMemo ? "Yes" : "No"} | useCallback: ${a.hasUseCallback ? "Yes" : "No"}`
    );
    logger.info("");
  });
  ...
  if (mediumPriority.length > 10) {
    logger.info(
      "ReactAnalysis",
      `  ... and ${mediumPriority.length - 10} more\n`
    );
  }
  ...
  logger.info(
    "ReactAnalysis",
    `1. Add React.memo() to ${highPriority.filter((a) => !a.hasMemo).length} high-priority components`
  );
  logger.info(
    "ReactAnalysis",
    `2. Add useCallback to ${withoutUseCallback.filter((a) => a.priority === "high").length} high-priority event handlers`
  );
  logger.info(
    "ReactAnalysis",
    `3. Review ${mediumPriority.length} medium-priority components for optimization`
  );
  logger.info("=".repeat(80));
  logger.info("");
  ...
  logger.info(
    "ReactAnalysis",
    "Detailed report saved to: src/scripts/component-analysis-report.json"
  );
