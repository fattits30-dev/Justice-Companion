...
export class DatabaseQueryAnalyzer {
  ...
  analyzeQuery(sql: string, params: unknown[] = []): QueryAnalysis {
    ...
    const recommendations = this.generateRecommendations(sql, plan, executionTime);
  ...
  private generateRecommendations(
    ...
    if (executionTime > 100) {
      recommendations.push(`Slow query (${executionTime.toFixed(2)}ms). Consider optimization.`);
    }
    
    // Check for missing indexes
    if (!plan.usesIndex && sql.toLowerCase().includes('where')) {
      recommendations.push('Query does not use an index. Consider adding one on filtered columns.');
    }
    ...
    if (sql.toLowerCase().includes(' or ')) {
      recommendations.push('OR conditions can prevent index usage. Consider restructuring with UNION.');
    }
    
    // Check for functions in WHERE
    if (/where.*\(.*\)/.test(sql.toLowerCase())) {
      recommendations.push('Functions in WHERE clause prevent index usage. Consider computed columns.');
    }
   ...
   */
  benchmarkQuery(sql: string, params: unknown[] = [], iterations: number = 100): {
    avgTime: number;
    minTime: number;
    maxTime: number;
    totalTime: number;
  ...
  generateOptimizationReport(): {
    ...
    if (unindexedTables.length > 0) {
      recommendations.push(`Found ${unindexedTables.length} tables without indexes.`);
    }
    
    if (databaseSize > 100 * 1024 * 1024) {
      recommendations.push('Database is over 100MB. Consider archiving old data.');
    }
    
    // Check for vacuum opportunity
    const freelistCount = this.db.pragma('freelist_count', { simple: true }) as number;
    if (freelistCount > 1000) {
      recommendations.push('Database has fragmentation. Run VACUUM to reclaim space.');
    }
