...
describe("validatePasswordChange", () => {
    ...
    it("should return error when new password is too short", () => {
        const result = validatePasswordChange(validOldPassword, shortPassword, shortPassword);
        expect(result.isValid).toBe(false);
    ...
    it("should return error when new password lacks uppercase letter", () => {
        const result = validatePasswordChange(validOldPassword, noUppercasePassword, noUppercasePassword);
        expect(result.isValid).toBe(false);
        expect(result.error).toBe("Password must contain at least one uppercase letter");
    });
    it("should return error when new password lacks lowercase letter", () => {
        const result = validatePasswordChange(validOldPassword, noLowercasePassword, noLowercasePassword);
        expect(result.isValid).toBe(false);
    ...
    it("should return error when new password lacks number", () => {
        const result = validatePasswordChange(validOldPassword, noDigitPassword, noDigitPassword);
        expect(result.isValid).toBe(false);
        expect(result.error).toBe("Password must contain at least one number");
    });
    it("should return error when passwords do not match", () => {
        const result = validatePasswordChange(validOldPassword, validNewPassword, mismatchedPassword);
        expect(result.isValid).toBe(false);
    ...
    it("should validate password with exactly 12 characters", () => {
        const result = validatePasswordChange(validOldPassword, twelveCharacterPassword, twelveCharacterPassword);
        expect(result.isValid).toBe(true);
    });
    it("should validate password with special characters", () => {
        const result = validatePasswordChange(validOldPassword, specialCharacterPassword, specialCharacterPassword);
        expect(result.isValid).toBe(true);
    });
    it("should validate very long password", () => {
        const result = validatePasswordChange(validOldPassword, veryLongPassword, veryLongPassword);
        expect(result.isValid).toBe(true);
    });
});
