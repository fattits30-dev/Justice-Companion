...
export const fileDownloadSchema = z
  .object({
    ...
    fileName: z
      ...
      .max(
        MAX_FILENAME_LENGTH,
        `File name must be less than ${MAX_FILENAME_LENGTH} characters`,
      )
...
export const fileEmailSchema = z
  .object({
    filePaths: z
      ...
      .max(
        MAX_FILES_PER_UPLOAD,
        `Cannot email more than ${MAX_FILES_PER_UPLOAD} files at once`,
      )
      .refine((paths) => {
    ...
    body: z
      ...
      .max(
        MAX_STRING_LENGTH,
        `Body must be less than ${MAX_STRING_LENGTH} characters`,
      )
      .trim()
      .optional(),
  })
