    ...
    legislation: z
      .array(
        z.object({
          ...
          section: z
            .string()
            .max(100, "Section reference is too long")
            .trim()
            .optional(),
          ...
          url: z
            ...
            .refine((url) => {
              // Normalize URL for security check (lowercase, decode, remove null bytes)
              const normalized = decodeURIComponent(
                url.toLowerCase().replace(/\0/g, ""),
              );
              // Ensure URL starts with exact protocol and domain (no subdomain abuse)
              const validPrefixes = [
    ...
    caseLaw: z
      .array(
        z.object({
          ...
          outcome: z
            .string()
            .max(500, "Outcome description is too long")
            .trim()
            .optional(),
  ...
  .object({
    errorData: z
      .object({
        ...
        errorInfo: z
          .string()
          .max(MAX_STRING_LENGTH, "Error info is too long")
          .trim(),

        componentStack: z
          .string()
          .max(MAX_STRING_LENGTH, "Component stack is too long")
          .trim(),

        timestamp: z
          .string()
          .regex(PATTERNS.ISO_DATE, "Invalid timestamp format"),
