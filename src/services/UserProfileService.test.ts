...
describe('UserProfileService', () => {
  ...
  describe('getProfile()', () => {
    ...
    it('should log PII access when reading encrypted fields', () => {
      ...
      const accessLog = logs.find((log: any) => log.event_type === 'profile.pii_access');
  ...
  describe('updateProfile()', () => {
    ...
    it('should log profile update event', () => {
      ...
      const logs = (auditLogger as any).getAllLogs();
      const updateLog = logs.find((log: any) => log.event_type === 'profile.update');
  ...
  describe('GDPR Compliance', () => {
    it('should encrypt PII fields (name and email) at rest', () => {
      ...
      const db = testDb.getDatabase();
      const raw = db.prepare('SELECT name, email FROM user_profile WHERE id = 1').get() as any;
    ...
    it('should maintain audit trail of profile changes', () => {
      ...
      const logs = (auditLogger as any).getAllLogs();
      const updateLogs = logs.filter((log: any) => log.event_type === 'profile.update');
