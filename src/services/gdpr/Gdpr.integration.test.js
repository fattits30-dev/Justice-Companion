...
describe("GDPR Integration Tests", () => {
    ...
    beforeEach(() => {
        ...
        db.prepare(`INSERT INTO consents (user_id, consent_type, granted, version) VALUES (?, ?, ?, ?)`).run(testUserId, "data_processing", 1, "1.0");
        db.prepare(`INSERT INTO consents (user_id, consent_type, granted, version) VALUES (?, ?, ?, ?)`).run(testUserId, "ai_processing", 1, "1.0");
    });
    afterEach(() => {
        testDb.cleanup();
    ...
    describe("Article 20: Data Portability (Export)", () => {
        ...
        it("should export user data with decryption of encrypted fields", async () => {
            ...
            const encryptedDescription = encryptionService.encrypt("Sensitive legal case details");
            db.prepare(`INSERT INTO cases (user_id, title, description, case_type) VALUES (?, ?, ?, ?)`).run(testUserId, "Test Case", JSON.stringify(encryptedDescription), "employment");
            // Export data
            const result = await gdprService.exportUserData(testUserId);
            // Should have 1 case
        ...
        it("should export all 15 tables with correct counts", async () => {
            ...
            const caseResult = db
                .prepare(`INSERT INTO cases (user_id, title, case_type) VALUES (?, ?, ?)`)
                .run(testUserId, "Case 1", "employment");
            const caseId = caseResult.lastInsertRowid;
            db.prepare(`INSERT INTO evidence (case_id, user_id, title, evidence_type, content) VALUES (?, ?, ?, ?, ?)`).run(caseId, testUserId, "Evidence 1", "document", "Evidence content placeholder");
            db.prepare(`INSERT INTO chat_conversations (user_id, case_id, title) VALUES (?, ?, ?)`).run(testUserId, caseId, "Chat 1");
            db.prepare(`INSERT INTO user_facts (case_id, user_id, fact_content, fact_type) VALUES (?, ?, ?, ?)`).run(caseId, testUserId, "Fact 1", "personal");
            db.prepare(`INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)`).run("session123", testUserId, "2025-12-31");
            // Export
            const result = await gdprService.exportUserData(testUserId);
            // Verify counts
    ...
    describe("Article 17: Right to Erasure (Delete)", () => {
        beforeEach(() => {
            ...
            const evidenceResult = db
                .prepare(`INSERT INTO evidence (case_id, user_id, title, evidence_type, content) VALUES (?, ?, ?, ?, ?)`)
                .run(caseId, testUserId, "Evidence 1", "document", "Evidence content placeholder");
            const evidenceId = evidenceResult.lastInsertRowid;
            const timelineResult = db
                .prepare(`INSERT INTO timeline_events (case_id, user_id, event_date, title) VALUES (?, ?, ?, ?)`)
                .run(caseId, testUserId, "2025-01-15", "Timeline Event 1");
            const timelineId = timelineResult.lastInsertRowid;
            db.prepare(`INSERT INTO event_evidence (event_id, evidence_id) VALUES (?, ?)`).run(timelineId, evidenceId);
            db.prepare(`INSERT INTO legal_issues (case_id, user_id, title) VALUES (?, ?, ?)`).run(caseId, testUserId, "Employment Issue");
            db.prepare(`INSERT INTO actions (case_id, title, description) VALUES (?, ?, ?)`).run(caseId, "File complaint", "File employment complaint");
            db.prepare(`INSERT INTO notes (case_id, user_id, content) VALUES (?, ?, ?)`).run(caseId, testUserId, "Important note");
            db.prepare(`INSERT INTO case_facts (case_id, user_id, fact_content, fact_category) VALUES (?, ?, ?, ?)`).run(caseId, testUserId, "Key fact", "evidence");
            db.prepare(`INSERT INTO chat_conversations (user_id, case_id, title) VALUES (?, ?, ?)`).run(testUserId, caseId, "Chat");
            db.prepare(`INSERT INTO user_facts (case_id, user_id, fact_content, fact_type) VALUES (?, ?, ?, ?)`).run(caseId, testUserId, "User fact", "personal");
            db.prepare(`INSERT INTO sessions (id, user_id, expires_at) VALUES (?, ?, ?)`).run("session123", testUserId, "2025-12-31");
        ...
        it("should require explicit confirmation", async () => {
            await expect(gdprService.deleteUserData(testUserId, { confirmed: false })).rejects.toThrow("explicit confirmation");
        });
        ...
        it("should respect foreign key constraints", async () => {
            ...
            await expect(gdprService.deleteUserData(testUserId, { confirmed: true })).resolves.toBeDefined();
        });
    ...
    describe("Rate Limiting", () => {
        it("should allow 5 exports per 24 hours", async () => {
            ...
            // 6th should fail
            await expect(gdprService.exportUserData(testUserId)).rejects.toThrow("Rate limit exceeded");
        });
        it("should allow 1 deletion per 30 days", async () => {
            ...
            db.prepare(`INSERT INTO consents (user_id, consent_type, granted, version) VALUES (?, ?, ?, ?)`).run(newUserId, "data_processing", 1, "1.0");
    ...
    describe("Consent Management", () => {
        it("should require data_processing consent for export", async () => {
            // Remove consent
            db.prepare(`DELETE FROM consents WHERE consent_type = ?`).run("data_processing");
            await expect(gdprService.exportUserData(testUserId)).rejects.toThrow("consent");
        });
        it("should require data_processing consent for delete", async () => {
            // Remove consent
            db.prepare(`DELETE FROM consents WHERE consent_type = ?`).run("data_processing");
            await expect(gdprService.deleteUserData(testUserId, { confirmed: true })).rejects.toThrow("consent");
        });
    });
});
