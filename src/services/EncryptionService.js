...
class EncryptionService {
    ...
    version = 1;
    ivLength = 12; // 96 bits is standard for GCM
    key;
    /**
    ...
    constructor(key) {
        ...
        if (this.key.length !== 32) {
            throw new Error(`Encryption key must be exactly 32 bytes (256 bits), got ${this.key.length} bytes`);
        }
    }
    /**
    ...
    encrypt(plaintext) {
        ...
        try {
            ...
            const iv = node_crypto_1.default.randomBytes(this.ivLength);
            // Create cipher
            const cipher = node_crypto_1.default.createCipheriv(this.algorithm, this.key, iv);
            // Encrypt data
    ...
    decrypt(encryptedData) {
        ...
        try {
            ...
            // Create decipher
            const decipher = node_crypto_1.default.createDecipheriv(this.algorithm, this.key, iv);
    ...
    isEncrypted(data) {
        ...
        const obj = data;
        return (typeof obj.algorithm === 'string' &&
     ...
     */
    static generateKey() {
        return node_crypto_1.default.randomBytes(32); // 32 bytes = 256 bits
    }
    /**
     ...
     */
    rotateKey(oldEncryptedData, newService) {
