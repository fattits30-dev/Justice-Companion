...
describe("AuditLogger E2E", () => {
    ...
    `);
        ...
        if (row && row.description) {
            try {
                const encryptedData = JSON.parse(row.description);
    ...
    `);
        ...
        const evidenceInfo = db
            .prepare("SELECT case_id as caseId, evidence_type as evidenceType FROM evidence WHERE id = ?")
            .get(id);
    ...
    describe("Performance Tests", () => {
        ...
        it("queries large audit log efficiently", () => {
            ...
            for (let i = 0; i < 1000; i++) {
                const eventTypes = ["case.create", "case.update", "evidence.create"];
                auditLogger.log({
    ...
    describe("Tamper Detection in Real Scenarios", () => {
        it("detects tampering attempt in multi-step workflow", () => {
            ...
            const logs = auditLogger.query({ eventType: "evidence.create" });
            expect(logs).toHaveLength(1);
            db.prepare("UPDATE audit_logs SET event_type = ? WHERE id = ?").run("evidence.delete", logs[0].id);
