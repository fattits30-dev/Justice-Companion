...
describe.sequential("AuditLogger", () => {
    ...
    describe("Chain Integrity", () => {
        ...
        it("detects tampered integrity hash", () => {
            ...
            db.prepare("UPDATE audit_logs SET integrity_hash = ? WHERE id = (SELECT id FROM audit_logs ORDER BY ROWID ASC LIMIT 1)").run("TAMPERED_HASH_1234567890123456789012345678901234567890123456789012");
            const report = auditLogger.verifyIntegrity();
        ...
        it("detects broken chain link", () => {
            ...
            db.prepare("UPDATE audit_logs SET previous_log_hash = ? WHERE id = (SELECT id FROM audit_logs ORDER BY ROWID DESC LIMIT 1 OFFSET 1)").run("BROKEN_LINK_1234567890123456789012345678901234567890123456789012");
            const report = auditLogger.verifyIntegrity();
            expect(report.valid).toBe(false);
        });
        it("detects tampered event data", () => {
            ...
            db.prepare("UPDATE audit_logs SET event_type = ? WHERE id = (SELECT id FROM audit_logs ORDER BY ROWID ASC LIMIT 1)").run("case.delete");
            const report = auditLogger.verifyIntegrity();
            expect(report.valid).toBe(false);
            expect(report.brokenAt).toBe(0);
        });
    ...
    describe("Edge Cases", () => {
        ...
        it("serializes complex details objects", () => {
            ...
            expect(details.nested.value).toBe(123);
        });
        ...
        it("handles very long detail strings", () => {
            ...
            const logs = auditLogger.query();
            const details = logs[0].details;
            expect(details.longField.length).toBe(10000);
        });
        it("handles special characters in fields", () => {
            ...
            const logs = auditLogger.query();
            const details = logs[0].details;
            expect(details.special).toContain('"quotes"');
            expect(details.special).toContain("'apostrophes'");
        });
    ...
    describe("Large Chain Verification", () => {
        ...
        it("detects tampering in middle of large chain", () => {
            ...
            db.prepare("UPDATE audit_logs SET integrity_hash = ? WHERE id = ?").run("TAMPERED_HASH_1234567890123456789012345678901234567890123456789012", middleLog.id);
            const report = auditLogger.verifyIntegrity();
            expect(report.valid).toBe(false);
        });
    });
