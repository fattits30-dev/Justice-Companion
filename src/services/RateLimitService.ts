...
export class RateLimitService {
  ...
  public checkRateLimit(username: string): RateLimitResult {
    ...
    if (attempt.lockedUntil && attempt.lockedUntil > now) {
      ...
      console.warn(`[RateLimitService] Rate limit exceeded for ${normalizedUsername}. Attempts: ${attempt.count}, Lock time remaining: ${remainingSeconds}s`);
    ...
    if (attempt.count >= this.MAX_ATTEMPTS) {
      ...
      console.warn(`[RateLimitService] Account locked for ${normalizedUsername}. Attempts: ${attempt.count}, Lock duration: ${this.LOCK_DURATION_MS / 1000}s`);
    ...
    if (!attempt) {
      ...
      if (attempt.firstAttemptAt < windowStart) {
        ...
        if (attempt.count >= this.MAX_ATTEMPTS && !attempt.lockedUntil) {
          ...
          console.error(`[RateLimitService] BRUTE FORCE DETECTED for ${normalizedUsername}. Account locked for ${this.LOCK_DURATION_MS / 1000}s`);
        }
      }
