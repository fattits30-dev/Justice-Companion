...
describe("NotificationService", () => {
    ...
    beforeEach(() => {
        ...
        preferencesRepo = new NotificationPreferencesRepository({});
    ...
    describe("createNotification", () => {
        ...
        it("should throw error when notification type is disabled", async () => {
            ...
            await expect(service.createNotification(input)).rejects.toThrow("Notification type deadline_reminder is disabled");
        });
        it("should throw error during quiet hours", async () => {
            ...
            await expect(service.createNotification(input)).rejects.toThrow("Notification blocked during quiet hours");
        });
    });
    ...
    describe("createSystemNotification", () => {
        it("should create system notification with appropriate type", async () => {
            ...
            const result = await service.createSystemNotification(1, "medium", "System Warning", "This is a system warning");
            expect(result.type).toBe("system_warning");
            expect(notificationRepo.create).toHaveBeenCalledWith(expect.objectContaining({
                type: "system_warning",
                severity: "medium",
            }));
            expect(logSpy).toHaveBeenCalledWith(expect.objectContaining({ eventType: "notification.create" }));
        });
    });
});
