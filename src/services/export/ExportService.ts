...
export class ExportService implements IExportService {
  ...
  async exportCaseToPDF(
    ...
    try {
      ...
      if (!hasAccess) {
        throw new Error(
          "Permission denied: User does not have access to this case",
        );
      }
      ...
      switch (fullOptions.template) {
        ...
        case "evidence-list": {
          ...
          pdfBuffer =
            await this.pdfGenerator.generateEvidenceList(evidenceData);
          break;
        }
        case "timeline-report": {
          const timelineData = this.prepareTimelineData(caseData);
          pdfBuffer =
            await this.pdfGenerator.generateTimelineReport(timelineData);
          break;
        }
        case "case-notes": {
      ...
      // Save PDF to file
      const filePath = await this.savePDFToFile(
        pdfBuffer,
        caseData.case,
        fullOptions,
      );
    ...
    try {
      ...
      switch (fullOptions.template) {
        ...
        case "evidence-list": {
          const evidenceData = this.prepareEvidenceData(caseData);
          docxBuffer =
            await this.docxGenerator.generateEvidenceList(evidenceData);
          break;
        }
        case "timeline-report": {
          const timelineData = this.prepareTimelineData(caseData);
          docxBuffer =
            await this.docxGenerator.generateTimelineReport(timelineData);
          break;
        }
        case "case-notes": {
          const notesData = this.prepareNotesData(caseData);
      ...
      // Save DOCX to file
      const filePath = await this.saveDOCXToFile(
        docxBuffer,
        caseData.case,
        fullOptions,
      );
  ...
  async exportTimelineReportToPDF(
    caseId: number,
    userId: number,
  ): Promise<ExportResult> {
    return this.exportCaseToPDF(caseId, userId, {
  ...
  private async gatherCaseData(
    ...
    const decryptedCase: Case = {
      ...
      description: caseData.description
        ? await this.encryption.decrypt(caseData.description)
        : null,
    };
    ...
    if (options.includeEvidence) {
      ...
      evidence = await Promise.all(
        rawEvidence.map(async (e) => ({
          ...
          filePath: e.filePath
            ? await this.encryption.decrypt(e.filePath)
            : null,
        })),
      );
    }
