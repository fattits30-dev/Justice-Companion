...
export class PDFGenerator {
  ...
  async generateCaseSummary(caseData: CaseExportData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(this.styles.body?.fontSize ?? 11)
          .fillColor("#000000")
          .text(
          ...
          .text(
            `Created: ${new Date(caseData.case.createdAt).toLocaleDateString()}`,
          )
          .text(
            `Last Updated: ${new Date(caseData.case.updatedAt).toLocaleDateString()}`,
          );
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (Object.keys(evidenceData.categorySummary).length > 0) {
          ...
          doc.fontSize(this.styles.body?.fontSize ?? 11).fillColor("#000000");

          Object.entries(evidenceData.categorySummary).forEach(
            ([category, count]) => {
  ...
  private addHeader(doc: PDFKit.PDFDocument, title: string): void {
    doc
      .fontSize(this.styles.title?.fontSize ?? 24)
    ...
    doc
      .moveTo(this.pageMargins.left, doc.y + 10)
  ...
  private addFooters(
    ...
    for (let i = 0; i < pages.count; i++) {
      ...
      // Add page number
      doc
        .fontSize(this.styles.footer?.fontSize ?? 9)
  ...
  private addEvidenceSection(
    ...
    evidence.forEach((item, index) => {
      ...
      if (item.obtainedDate) {
        doc.text(
          `Date Obtained: ${new Date(item.obtainedDate).toLocaleDateString()}`,
        );
      }
  ...
  private addDeadlinesSection(
    ...
    deadlines.forEach((deadline) => {
      ...
      doc
        ...
        .fillColor(color)
        .text(
          `${new Date(deadline.deadlineDate).toLocaleDateString()} - ${deadline.title}`,
          this.pageMargins.left,
          doc.y,
        );
