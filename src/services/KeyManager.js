...
class KeyManager {
    ...
    getKey() {
        ...
        if (!fs.existsSync(this.keyFilePath)) {
            throw new Error('Encryption key not found. Run migration script to move key from .env to safeStorage.');
        }
        ...
        if (this.cachedKey.length !== 32) {
            ...
            throw new Error(`Invalid encryption key: expected 32 bytes, got ${actualLength} bytes`);
        }
        return this.cachedKey;
    }
    /**
     ...
     */
    migrateFromEnv(envKey) {
        if (!this.safeStorage.isEncryptionAvailable()) {
            throw new Error('safeStorage encryption is not available. Cannot migrate key.');
        }
        // Validate key format
    ...
    generateNewKey() {
        if (!this.safeStorage.isEncryptionAvailable()) {
            throw new Error('safeStorage encryption is not available. Cannot generate key.');
        }
