...
describe("SessionPersistenceService (non-browser environments)", () => {
  ...
  const resetInstance = () => {
    (
      SessionPersistenceService as unknown as {
        instance?: SessionPersistenceService;
      }
    ).instance = undefined;
  };
  ...
  it("logs and swallows errors when clearing sessions fails", async () => {
    ...
    const ServiceWithAccess =
      SessionPersistenceService as typeof SessionPersistenceService & {
        getStorage(): Storage | null;
      };
    const getStorageSpy = vi
      .spyOn(ServiceWithAccess, "getStorage")
      .mockReturnValue(storageMock) as ReturnType<typeof vi.spyOn>;
