...
describe("AuthorizationService", () => {
    ...
    `);
        ...
        testUserId = db.prepare("SELECT last_insert_rowid() as id").get().id;
        insertUser.run("testadmin", "admin@example.com", "hash", "salt", "admin");
        testUserId2 = db.prepare("SELECT last_insert_rowid() as id").get().id;
        // Get role IDs
        adminRoleId = db.prepare("SELECT id FROM roles WHERE name = ?").get("admin").id;
        userRoleId = db.prepare("SELECT id FROM roles WHERE name = ?").get("user").id;
        viewerRoleId = db.prepare("SELECT id FROM roles WHERE name = ?").get("viewer").id;
