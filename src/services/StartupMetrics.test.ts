...
describe('StartupMetrics', () => {
  ...
  describe('logStartupMetrics', () => {
    it('should log formatted metrics to console', async () => {
      ...
      await new Promise(resolve => setTimeout(() => { metrics.recordPhase('loadingWindowShown'); resolve(null); }, 10));
      await new Promise(resolve => setTimeout(() => { metrics.recordPhase('criticalServicesReady'); resolve(null); }, 10));
      await new Promise(resolve => setTimeout(() => { metrics.recordPhase('mainWindowShown'); resolve(null); }, 10));
    ...
    it('should show recommendations for slow startup', async () => {
      ...
      await new Promise<void>(resolve => {
        setTimeout(() => {
          ...
          const warnOutput = consoleWarnSpy.mock.calls.map((call: any[]) => call[0]).join('\n');
