...
describe("AuthenticationService Integration - Rate Limiting", () => {
    ...
    describe("Rate Limiting on Login", () => {
        ...
        it("should block login after 5 failed attempts", async () => {
            ...
            await expect(authService.login("testuser", "WrongPassword123!")).rejects.toThrow(/Account temporarily locked/);
        });
        ...
        it("should rate limit even for non-existent users", async () => {
            ...
            // 6th attempt should be blocked
            await expect(authService.login("nonexistent", "Password123!")).rejects.toThrow(/Account temporarily locked/);
        });
        it("should rate limit inactive accounts", async () => {
            ...
            for (let i = 0; i < 5; i++) {
                await expect(authService.login("testuser", "TestPassword123!")).rejects.toThrow("Account is inactive");
            }
            // 6th attempt should be blocked
            await expect(authService.login("testuser", "TestPassword123!")).rejects.toThrow(/Account temporarily locked/);
        });
    });
