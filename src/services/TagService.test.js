...
describe("TagService", () => {
    describe("createTag", () => {
        ...
        it("should allow same tag name for different users", () => {
            // Create second user
            db.prepare("INSERT INTO users (id, username, email, password_hash) VALUES (?, ?, ?, ?)").run(2, "user2", "user2@example.com", "hashed_password");
            const input = {
                name: "Same Name",
                color: "#10B981",
            };
    ...
    describe("getTags", () => {
        ...
        it("should return tags with usage counts", () => {
            ...
            // Create evidence
            db.prepare("INSERT INTO evidence (id, user_id, case_id, title) VALUES (?, ?, ?, ?)").run(1, testUserId, 1, "Evidence 1");
            // Tag evidence
    ...
    describe("searchByTags", () => {
        it("should find evidence with specified tags (AND logic)", () => {
            ...
            // Create evidence
            db.prepare("INSERT INTO evidence (id, user_id, case_id, title) VALUES (?, ?, ?, ?)").run(1, testUserId, 1, "Evidence 1");
            db.prepare("INSERT INTO evidence (id, user_id, case_id, title) VALUES (?, ?, ?, ?)").run(2, testUserId, 1, "Evidence 2");
        ...
        it("should only return evidence owned by user", () => {
            ...
            db.prepare("INSERT INTO evidence (id, user_id, case_id, title) VALUES (?, ?, ?, ?)").run(1, testUserId, 1, "Evidence 1");
            // Evidence for user 2
            db.prepare("INSERT INTO evidence (id, user_id, case_id, title) VALUES (?, ?, ?, ?)").run(2, 2, 1, "Evidence 2");
            tagService.tagEvidence(1, tag.id, testUserId);
