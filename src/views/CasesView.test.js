...
import { fireEvent, render, screen, waitFor, within, } from "../test-utils/test-utils.tsx";
...
async function getDeleteButtonForCase(caseTitle) {
...
describe("CasesView", () => {
    beforeEach(() => {
        ...
        apiClient.auth.getSession = vi.fn().mockResolvedValue({
            success: true,
            data: {
                user: {
                    id: "1",
    ...
    it("renders an error state and allows retry", async () => {
        ...
        await waitFor(() => expect(screen.getByText("Employment Dispute")).toBeInTheDocument());
    });
    it("shows empty state when there are no cases", async () => {
        apiClient.cases.list = vi.fn().mockResolvedValue({
            success: true,
            data: { items: [], total: 0, limit: 50, offset: 0, hasMore: false },
        });
        render(<CasesView />);
        await screen.findByText("No cases yet");
        expect(screen.getByText("Create your first case to keep everything organised.")).toBeInTheDocument();
    });
    ...
    it("allows creating a new case", async () => {
        ...
        apiClient.cases.create = vi.fn().mockResolvedValue({
            success: true,
            data: createdCase,
        });
        render(<CasesView />);
        ...
        await waitFor(() => expect(apiClient.cases.create).toHaveBeenCalledWith({
            title: "Consumer Rights Claim",
            description: "Faulty product dispute",
            caseType: "consumer",
        }));
    ...
    it("confirms before deleting a case", async () => {
        ...
        apiClient.cases.delete = vi.fn().mockResolvedValue({
            success: true,
        });
