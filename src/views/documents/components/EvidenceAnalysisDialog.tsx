...
import type { EvidenceAnalysisResponse, EvidenceGap, EvidenceImportance } from '../../../types/ai-analysis.ts';
...
export function EvidenceAnalysisDialog({ onClose, analysis, isLoading }: EvidenceAnalysisDialogProps) {
  ...
  const criticalGaps = analysis.gaps.filter((gap) => gap.importance === 'critical');
  const importantGaps = analysis.gaps.filter((gap) => gap.importance === 'important');
  const helpfulGaps = analysis.gaps.filter((gap) => gap.importance === 'helpful');
        ...
        <Card variant="glass" className="flex flex-col max-h-[90vh]">
          ...
          <div className="flex items-start justify-between p-6 border-b border-gray-700">
            <div className="flex items-start gap-4">
              ...
              <div>
                <h2 className="text-2xl font-bold text-white">Evidence Analysis</h2>
          ...
          <div className="space-y-6 p-6 overflow-y-auto flex-1">
              ...
              <Card variant="default" className="p-4">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                  <div className="text-center p-4 rounded-lg bg-primary-900/50">
                    ...
                    <p className={`text-3xl font-bold ${getStrengthColor(analysis.strength)}`}>
              ...
              <section>
                ...
                <div className="space-y-3">
                  {criticalGaps.map((gap: EvidenceGap, index: number) => (
                    <Card key={index} variant="default" className="p-4 border-l-4 border-red-500">
              ...
              <section>
                ...
                <div className="space-y-3">
                  {importantGaps.map((gap: EvidenceGap, index: number) => (
                    <Card key={index} variant="default" className="p-4 border-l-4 border-orange-500">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <h4 className="font-medium text-white">{gap.description}</h4>
              ...
              <section>
                ...
                <div className="space-y-3">
                  {helpfulGaps.map((gap: EvidenceGap, index: number) => (
                    <Card key={index} variant="default" className="p-4 border-l-4 border-yellow-500">
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <h4 className="font-medium text-white">{gap.description}</h4>
            ...
            {/* Disclaimer */}
            <Card variant="default" className="p-4 bg-yellow-900/20 border-yellow-700/50">
              <p className="text-xs text-yellow-200 italic">{analysis.disclaimer}</p>
            </Card>
          </div>
