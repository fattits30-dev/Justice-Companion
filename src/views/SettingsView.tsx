...
type AIProvider = 'openai' | 'groq' | 'anthropic' | 'google' | 'cohere' | 'mistral';
...
export function SettingsView() {
  ...
  useEffect(() => {
    const loadConfig = async () => {
      try {
        ...
        const hasKey = await window.justiceAPI.secureStorageHas(`${selectedProvider}_api_key`);
        ...
        // Load model
        const modelResult = await window.justiceAPI.secureStorageGet(`${selectedProvider}_model`);
        ...
        if (currentProvider.requiresOrganization) {
          const orgResult = await window.justiceAPI.secureStorageGet(`${selectedProvider}_organization`);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (result.success) {
        setSaveMessage({ type: 'success', text: `${currentProvider.name} API key saved!` });
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (hasKey.success && hasKey.data) {
        ...
        setTestMessage({ type: 'error', text: 'No API key configured. Save your key first.' });
      }
    } catch (error) {
      setTestMessage({
        type: 'error',
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 bg-blue-900/30 border-l-4 border-blue-500 rounded-lg p-4">
          <div className="flex items-start gap-3">
            <svg className="w-6 h-6 text-blue-500 flex-shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <p className="font-semibold text-blue-200 mb-1">Your API keys stay private</p>
              <p className="text-sm text-blue-100/80">
                All API keys are encrypted and stored locally on YOUR computer. We don't send them anywhere except to the AI provider you choose.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700 mb-6">
          <h2 className="text-xl font-semibold mb-4">Choose Your AI Provider</h2>
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-2">{currentProvider.apiKeyLabel}</label>
              <input
                type="password"
                value={apiKey}
            ...
            <div className="flex gap-3">
              <button
                onClick={handleSave}
                disabled={isSaving}
                className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-700 disabled:cursor-not-allowed px-6 py-3 rounded-lg font-medium transition-colors"
              >
        ...
        <div className="bg-gray-800 rounded-lg p-6 border border-gray-700">
          <h2 className="text-xl font-semibold mb-4">About Justice Companion</h2>
          <p className="text-gray-400 mb-4">
            Justice Companion helps you organize your legal case and understand UK employment law.
            It's built for real people dealing with real problems—not lawyers in fancy suits.
          </p>
          <p className="text-sm text-gray-500">
            Version 1.0.0 • Built with privacy first • All your data stays on YOUR computer
          </p>
        </div>
      </div>
    </div>
