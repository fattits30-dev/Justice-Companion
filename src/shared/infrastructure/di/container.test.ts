...
describe("DI Container", () => {
  ...
  describe("Core Infrastructure", () => {
    ...
    it("should resolve EncryptionService as singleton", () => {
      const service1 = container.get<IEncryptionService>(
        TYPES.EncryptionService,
      );
      const service2 = container.get<IEncryptionService>(
        TYPES.EncryptionService,
      );
  ...
  describe("Services", () => {
    ...
    it("should inject repository and database into CaseService", () => {
      ...
      testDb
        ...
        .run("testuser", "test@example.com", "hash", "salt", "user");
  ...
  describe("Container Configuration", () => {
    ...
    it("should support custom encryption keys in test mode", () => {
      const customKey = Buffer.from(
        "custom-test-key-32-bytes-long!!!",
      ).toString("base64");
  ...
  describe("Dependency Graph", () => {
    it("should resolve complex dependency chains", () => {
      ...
      testDb
        ...
        .run("chainuser", "chain@example.com", "hash", "salt", "user");

      const userId = testDb
        .prepare("SELECT id FROM users WHERE username = ?")
        .get("chainuser") as { id: number };
