...
const resolveDatabasePath = () => {
    ...
    if (process.versions?.electron && electron_1.app && typeof electron_1.app.getPath === 'function') {
        try {
            ...
            error_logger_1.errorLogger.logError(error, { context: 'database-path-resolution' });
        }
    }
    ...
    getDatabase() {
        if (!this.db) {
            try {
                ...
                error_logger_1.errorLogger.logError('Database initialized successfully', { path: dbPath });
            }
            catch (error) {
                error_logger_1.errorLogger.logError(error, { context: 'database-init' });
                throw error;
            }
        }
        return this.db;
    ...
    close() {
        if (this.db) {
            try {
                this.db.close();
                this.db = null;
                error_logger_1.errorLogger.logError('Database closed successfully', {});
            }
            catch (error) {
                error_logger_1.errorLogger.logError(error, { context: 'database-close' });
            }
        }
    }
    runMigrations() {
      ...
      `);
            error_logger_1.errorLogger.logError('Migrations table ready', {});
        }
        catch (error) {
            error_logger_1.errorLogger.logError(error, { context: 'migrations-table-creation' });
            throw error;
        }
    }
    /**
