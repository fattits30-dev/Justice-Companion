...
function ensureMigrationsTable() {
    const db = (0, database_ts_1.getDb)();
    db.exec(`
    ...
    try {
        ...
        for (const file of migrationFiles) {
            if (!appliedNames.includes(file)) {
                ...
                error_logger_ts_1.errorLogger.logError(`Running migration: ${file}`, { type: 'info' });
          ...
          `).run(file, checksum, duration);
                ...
                applyMigration();
                error_logger_ts_1.errorLogger.logError(`Migration completed: ${file} (${Date.now() - startTime}ms)`, {
                    type: 'info',
                });
            }
            else {
                ...
                if (applied && applied.checksum !== currentChecksum) {
                    error_logger_ts_1.errorLogger.logError(`WARNING: Migration ${file} has been modified after being applied! ` +
                        `Original checksum: ${applied.checksum}, Current: ${currentChecksum}`, { type: 'warn' });
                }
            }
        }
        error_logger_ts_1.errorLogger.logError('All migrations completed', { type: 'info' });
    }
    catch (error) {
        error_logger_ts_1.errorLogger.logError(error, { context: 'run-migrations' });
        throw error;
    }
}
/**
...
function rollbackMigration(migrationName) {
    ...
    try {
        ...
        error_logger_ts_1.errorLogger.logError(`Rolling back migration: ${migrationName}`, { type: 'info' });
        const startTime = Date.now();
      ...
      `).run(migrationName);
        ...
        rollback();
        error_logger_ts_1.errorLogger.logError(`Rollback completed: ${migrationName} (${Date.now() - startTime}ms)`, {
            type: 'info',
        });
    }
    catch (error) {
        error_logger_ts_1.errorLogger.logError(error, { context: 'rollback-migration' });
        throw error;
    }
}
/**
...
if (import.meta.url === `file://${process.argv[1]}`) {
    try {
        ...
        logger_ts_1.logger.error('App', '❌ Migration failed:', { error: error });
        process.exit(1);
    }
}
