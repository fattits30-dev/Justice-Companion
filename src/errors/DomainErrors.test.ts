...
describe('DomainErrors', () => {
  describe('Base DomainError', () => {
    it('creates error with correct properties', () => {
      const error = new DomainError('TEST_ERROR', 'Test message', 400, { detail: 'test' });
  ...
  describe('AI Service Errors', () => {
    it('creates AINotConfiguredError', () => {
      ...
      expect(error.message).toBe('OpenAI not configured. Please set your API key in Settings.');
    });
  });

  describe('GDPR Compliance Errors', () => {
    it('creates GdprComplianceError with article information', () => {
      const error = new GdprComplianceError(17, 'Right to erasure', 'User consent required');
      expect(error.code).toBe('GDPR_COMPLIANCE_ERROR');
      expect(error.statusCode).toBe(451);
      expect(error.message).toBe('GDPR Article 17 - Right to erasure: User consent required');
      expect(error.context).toEqual({ article: 17, requirement: 'Right to erasure' });
    });
  });
  ...
  describe('Utility Functions', () => {
    ...
    it('createErrorResponse formats for IPC', () => {
      ...
      expect(response.error?.message).toBe('Validation failed for username: Too short');
