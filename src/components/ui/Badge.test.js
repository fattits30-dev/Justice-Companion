...
const dotSizeExpectations = {
    sm: ["w-1.5", "h-1.5"],
    md: ["w-2", "h-2"],
    lg: ["w-2.5", "h-2.5"],
};
const iconWrapperExpectations = {
    sm: ["w-3", "h-3"],
    md: ["w-3.5", "h-3.5"],
    lg: ["w-4", "h-4"],
};
const neutralVariantClasses = variantExpectations.find(([variant]) => variant === "neutral")?.[1] ?? [];
describe("Badge", () => {
    describe("rendering", () => {
        it("renders with default props", () => {
            render(<Badge>Default Badge</Badge>);
            const badge = screen.getByText("Default Badge");
            expect(badge).toHaveClass("inline-flex", "items-center", "justify-center", "font-medium", "rounded-md", "border", "transition-all", "duration-200", "backdrop-blur-sm", ...sizeExpectations.md, ...neutralVariantClasses);
        });
        it("renders children", () => {
    ...
    describe("variants", () => {
        ...
        it("applies glow shadow when enabled", () => {
            render(<Badge variant="success" glow>
          Glowing Badge
        </Badge>);
    ...
    describe("sizes", () => {
        ...
        Object.entries(sizeExpectations).forEach(([size, classes]) => {
    ...
    describe("dot indicator", () => {
        ...
        it("renders dot when enabled", () => {
            ...
            expect(dot).toHaveClass("rounded-full", ...dotSizeExpectations.md, "bg-gray-500");
        });
        dotColorExpectations.forEach(([variant, color]) => {
            it(`applies ${variant} dot color`, () => {
                render(<Badge dot variant={variant}>
            {variant}
          </Badge>);
        ...
        Object.entries(dotSizeExpectations).forEach(([size, classes]) => {
    ...
    describe("glow effect", () => {
        it("does not apply glow by default", () => {
            ...
            expect(badge).not.toHaveClass("shadow-success", "shadow-warning", "shadow-danger");
        });
        it("applies variant-specific glow class", () => {
            render(<Badge glow variant="warning">
          Warning Glow
        </Badge>);
    ...
    describe("icon", () => {
        ...
        Object.entries(iconWrapperExpectations).forEach(([size, classes]) => {
            it(`applies ${size} icon wrapper size`, () => {
                const TestIcon = () => <svg data-testid="badge-icon"/>;
                render(<Badge icon={<TestIcon />} size={size}>
            Icon Wrapper
          </Badge>);
    ...
    describe("accessibility", () => {
        it("forwards aria props", () => {
            render(<Badge aria-label="status-badge" role="status">
          Accessible Badge
        </Badge>);
