...
export class CaseType {
  ...
  private constructor(
    private readonly value: 'employment' | 'housing' | 'consumer' | 'family' | 'debt' | 'other'
  ) {}

  static create(type: string): CaseType {
    if (!CaseType.VALID_TYPES.includes(type as any)) {
      throw new Error(`Invalid case type: ${type}. Must be one of ${CaseType.VALID_TYPES.join(', ')}`);
    }
    return new CaseType(type as 'employment' | 'housing' | 'consumer' | 'family' | 'debt' | 'other');
  }
  ...
  getValue(): 'employment' | 'housing' | 'consumer' | 'family' | 'debt' | 'other' {
    return this.value;
  }
