 ...
 * 2. Frontend is running on http://localhost:5178
...
test.describe('Document Upload - Simplified Flow', () => {
  test('complete document upload and analysis workflow', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e-tests/screenshots/step1-homepage.png', fullPage: true });
    ...
    if (currentUrl.includes('/login') || currentUrl.includes('/register')) {
      ...
      await page.screenshot({ path: 'e2e-tests/screenshots/step2-login.png', fullPage: true });
    ...
    await page.screenshot({ path: 'e2e-tests/screenshots/step2-after-auth.png', fullPage: true });

    // STEP 3: Navigate to chat page
    console.log('[3/5] Navigating to chat page...');
    await page.goto(`${APP_URL}/chat`, { waitUntil: 'networkidle', timeout: 30000 });
    await page.waitForTimeout(2000);
    await page.screenshot({ path: 'e2e-tests/screenshots/step3-chat-page.png', fullPage: true });
    ...
    const uploadButton = page.locator('button[title*="Upload document" i]').first();
    ...
    console.log('    File selected, upload and analysis starting automatically...');
    ...
    await page.screenshot({ path: 'e2e-tests/screenshots/step4-document-uploaded.png', fullPage: true });

    // STEP 5: Wait for AI analysis to complete (automatic after upload)
    console.log('[5/6] Waiting for AI analysis to complete (up to 90 seconds)...');
    ...
    try {
      ...
      await Promise.race([
        page.waitForSelector('text=/This is a|Analysis|employment|dismissal|case type/i', {
          timeout: 90000,
          state: 'visible'
        }),
        page.waitForSelector('[role="alert"]:has-text("error")', {
          timeout: 90000,
          state: 'visible'
        }).then(() => {
      ...
      await page.screenshot({ path: 'e2e-tests/screenshots/step5-analysis-complete.png', fullPage: true });

      // Verify results
      const analysisText = await page.locator('text=/This is a|Analysis/i').first().textContent();
      ...
      const createCaseButton = page.getByRole('button', { name: /create.*case/i });
      ...
      await page.screenshot({ path: 'e2e-tests/screenshots/step5-analysis-failed.png', fullPage: true });

      // Check for error messages
      const errorMsg = await page.locator('[role="alert"], .error').first().textContent().catch(() => null);
      if (errorMsg) {
