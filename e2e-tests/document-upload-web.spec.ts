 ...
 *
 * Application: http://172.26.160.1:5176
 * Backend: http://127.0.0.1:8001
 * Test Document: test-user-dismissal-letter.txt
 */

const APP_URL = 'http://localhost:5178';
const TEST_DOCUMENT_PATH = path.join(__dirname, '..', 'test-documents', 'test-user-dismissal-letter.txt');

// Test credentials - will create new account for testing
...
test.describe('Document Upload and Analysis Flow', () => {
  ...
  test.beforeEach(async ({ browser }) => {
    ...
    page.on('console', msg => {
      const text = msg.text();
      if (text.includes('[ENDPOINT]') || text.includes('Error') || text.includes('analysis')) {
  ...
  test('should complete full document upload and analysis flow', async () => {
    ...
    try {
      ...
      await page.waitForSelector('text=/Sign In|Create account/i', { timeout: 10000 });
      ...
      if (createLinkExists) {
        ...
        const usernameInput = await page.locator('input[name="username"]').or(page.locator('input[placeholder*="username" i]')).first();
        ...
        const registerButton = page.getByRole('button', { name: /create|register|sign up/i }).first();
        ...
        const loginButton = page.locator('button[type="submit"]').or(page.getByRole('button', { name: /sign in|login/i })).first();
      ...
      console.log(`[Step 2] Current URL after login/registration: ${currentUrl}`);

      if (currentUrl.includes('/login') || currentUrl.includes('/register')) {
        // Still on login/register page - failed
        console.log('[Step 2] Authentication failed - checking for error message...');
        await page.screenshot({
        ...
        console.log(`[Step 2] Error message: ${errorMessage || 'No error message found'}`);

        throw new Error(`Authentication failed. Error: ${errorMessage || 'Unknown error'}`);
      }
    ...
    try {
      ...
      if (!isVisible) {
        ...
        const uploadButton = await page.getByRole('button', { name: /upload|attach|document/i }).first().or(
      ...
      throw new Error('Upload button or file input not found. Please check the chat page UI.');
    }
    ...
    try {
      ...
      await page.waitForSelector('text=/upload.*success|file.*uploaded/i, [class*="success"], .text-green-500', {
        timeout: 10000,
        state: 'visible'
      }).catch(async (e) => {
        console.log('[Step 5] No explicit upload success message found, continuing...');
    ...
    try {
      ...
      const analyzeButton = await page.getByRole('button', { name: /analyze.*document|analyze/i }).or(
      ...
      if (!analyzeExists) {
        console.log('[Step 6] Analyze button not found, checking page state...');
        await page.screenshot({
          path: 'e2e-tests/screenshots/07-no-analyze-button.png',
          fullPage: true
        });
        throw new Error('Analyze Document button not found. Upload may have failed or UI may differ.');
      }
    ...
    console.log('[Step 7] Waiting for AI analysis to complete (timeout: 60s)...');
    ...
    try {
      ...
      const resultOrError = await Promise.race([
        ...
        page.waitForSelector('text=/This is a|Analysis|case type|claimant/i', {
          timeout: 60000,
          state: 'visible'
        }).then(() => 'success'),

        // Wait for error message
        page.waitForSelector('text=/error|failed|500|unable/i, [role="alert"]', {
          timeout: 60000,
          state: 'visible'
        }).then(() => 'error'),
      ]);
    ...
    try {
      ...
      const errorElements = await page.locator('[role="alert"].error, .text-red-500:has-text("error"), .text-destructive:has-text("error")').count();
      expect(errorElements).toBe(0);
      console.log('[Step 8] ✓ No error messages present');

      // Check for analysis text
      const analysisText = await page.locator('text=/This is a|Analysis|case type/i').first().textContent();
      expect(analysisText).toBeTruthy();
      expect(analysisText!.length).toBeGreaterThan(10);
      console.log(`[Step 8] ✓ Analysis text found: "${analysisText?.substring(0, 100)}..."`);

      // Check for "Create Case from Analysis" button
      const createCaseButton = await page.getByRole('button', { name: /create.*case.*from.*analysis/i }).or(
        page.locator('button:has-text("Create Case")')
      ).first();
      ...
      const fullAnalysis = await page.locator('text=/This is a/i').first().locator('..').textContent();
      console.log('\n=== Analysis Preview ===');
  ...
  test('should handle upload errors gracefully', async () => {
    ...
    const invalidFilePath = path.join(__dirname, '..', 'test-documents', 'test.exe');
    ...
    try {
      ...
      const errorVisible = await page.locator('text=/invalid.*file|unsupported.*type|not.*allowed/i').isVisible().catch(() => false);
      ...
      if (errorVisible) {
        ...
        console.log('[Test] ℹ No validation error shown (validation may not exist or file accepted)');
      }
