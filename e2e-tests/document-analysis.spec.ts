...
test.describe('Document Analysis - Name Mismatch Fix', () => {
  test('should not show warning when document name matches user name', async () => {
    ...
    const testFilePath = path.join(__dirname, '..', 'test-user-dismissal-letter.txt');
    ...
    console.log(`[Test] Analysis starts with: ${analysisText?.substring(0, 100)}...`);
    expect(analysisText).toBeTruthy();

    // Verify document claimant name in JSON is null (meaning no mismatch)
    const jsonBlock = await window.locator('pre:has-text("documentOwnershipMismatch")').textContent();
    if (jsonBlock) {
      const jsonData = JSON.parse(jsonBlock);
      console.log(`[Test] documentOwnershipMismatch: ${jsonData.documentOwnershipMismatch}`);
      console.log(`[Test] documentClaimantName: ${jsonData.documentClaimantName}`);
  ...
  test('sidebar item counts should match dashboard data', async () => {
    ...
    // Get dashboard stats
    const activeCasesText = await window.locator('text=Active Cases').locator('..').locator('text=/\\d+/').textContent();
    const documentsText = await window.locator('text=Evidence Items').locator('..').locator('text=/\\d+/').textContent();
    ...
    const sidebarCasesBadge = await window.locator('[aria-label="Cases"] [class*="badge"]').textContent();
    const sidebarDocsBadge = await window.locator('[aria-label="Documents"] [class*="badge"]').textContent();
