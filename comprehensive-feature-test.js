...
let userId = null;
let caseId = null;
const registrationUser = buildEphemeralTestUser("comprehensive-feature");
const fallbackLogin = {
  username: process.env.COMPREHENSIVE_TEST_EXISTING_USERNAME || "testuser",
  password:
    process.env.COMPREHENSIVE_TEST_EXISTING_PASSWORD ||
    deriveTestPassword("comprehensive-existing-user"),
};
...
async function runTests() {
  try {
    ...
    if (register.response.ok) {
      ...
      const login = await apiCall("/auth/login", {
        ...
        body: JSON.stringify({
          ...
          password: fallbackLogin.password,
        }),
      });
    ...
    if (dashboard.response.ok) {
      ...
      console.log(
        `   ✅ Total Evidence: ${dashboard.data.stats.total_evidence}`
      );
      console.log(
        `   ✅ Upcoming Deadlines: ${dashboard.data.stats.upcoming_deadlines}`
      );
      console.log(
        `   ✅ Overdue Deadlines: ${dashboard.data.stats.overdue_deadlines}\n`
      );
    }
    ...
    if (aiProviders.response.ok) {
      ...
      if (aiProviders.data.length > 0) {
        console.log(
          `   ✅ Active provider: ${aiProviders.data.find((p) => p.is_active)?.provider || "None"}\n`
        );
      } else {
    ...
    if (exportData.response.ok) {
      ...
      console.log(
        `   ✅ Cases exported: ${exportData.data.cases?.length || 0}`
      );
    ...
    if (dbInfo.response.ok) {
      console.log(
        `   ✅ Database type: ${dbInfo.data.database_type || "SQLite"}`
      );
      console.log(
        `   ✅ Database path: ${dbInfo.data.database_path ? "Configured" : "Default"}`
      );
