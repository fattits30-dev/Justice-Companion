...
class SecureStorageService:
    ...
    async def init(self) -> None:
        ...
        with self._init_lock:
            ...
            try:
                ...
                logger.error(f"[SecureStorage] Initialization failed: {e}")
                raise SecureStorageError(f"Failed to initialize secure storage: {e}")
    ...
    def _detect_keyring_backend(self) -> None:
        ...
        try:
            ...
            if backend_name == "fail.Keyring" or backend_name == "Keyring":
                ...
                logger.info(f"[SecureStorage] Using keyring backend: {backend_name}")
                self._encryption_available = True

            self._keyring_backend = backend

        except Exception as e:
            logger.warning(f"[SecureStorage] Failed to detect keyring backend: {e}")
            self._encryption_available = False
    ...
    async def set_api_key(self, key: str, value: str) -> None:
        ...
        try:
            ...
            logger.error(f"[SecureStorage] Failed to store key '{key}': {e}")
            raise SecureStorageError(f"Failed to store API key: {e}")
        except Exception as e:
            logger.error(f"[SecureStorage] Unexpected error storing key '{key}': {e}")
            raise SecureStorageError(f"Failed to store API key: {e}")
    ...
    async def get_api_key(self, key: str) -> Optional[str]:
        ...
        try:
            ...
            if value:
                logger.debug(f"[SecureStorage] Successfully retrieved key: {key}")
            else:
                logger.debug(f"[SecureStorage] Key not found: {key}")

            return value

        except KeyringError as e:
            logger.error(f"[SecureStorage] Failed to retrieve key '{key}': {e}")
            raise SecureStorageError(f"Failed to retrieve API key: {e}")
        except Exception as e:
            logger.error(f"[SecureStorage] Unexpected error retrieving key '{key}': {e}")
            raise SecureStorageError(f"Failed to retrieve API key: {e}")
    ...
    async def delete_api_key(self, key: str) -> None:
        ...
        try:
            ...
            logger.warning(f"[SecureStorage] Key not found for deletion: {key}")
            # Don't raise an exception for missing keys (matches TypeScript behavior)
        except KeyringError as e:
            logger.error(f"[SecureStorage] Failed to delete key '{key}': {e}")
            raise SecureStorageError(f"Failed to delete API key: {e}")
        except Exception as e:
            logger.error(f"[SecureStorage] Unexpected error deleting key '{key}': {e}")
            raise SecureStorageError(f"Failed to delete API key: {e}")
    ...
    async def has_api_key(self, key: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"[SecureStorage] Error checking key existence '{key}': {e}")
            return False
    ...
    async def clear_all(self) -> None:
        ...
        for key in common_keys:
            try:
                ...
                logger.warning(f"[SecureStorage] Failed to delete key '{key}': {e}")
