...
_RuntimeAsyncInferenceClient: Optional[Any] = _safe_import_attr(
...
class AIProviderConfig(BaseModel):
    ...
    max_tokens: int = Field(
        4096, gt=0, le=200000, description="Maximum tokens to generate"
    )
...
class StreamingCallbacks(BaseModel):
    ...
    on_complete: Optional[str] = Field(
        None, description="Callback when streaming completes"
    )
    on_error: Optional[str] = Field(None, description="Callback for errors")
    on_function_call: Optional[str] = Field(
        None, description="Callback for function calls"
    )
...
class AISDKService:
    ...
    def _initialize_client(self) -> None:
        ...
        try:
            if provider == AIProviderType.ANTHROPIC:
                if _RuntimeAsyncAnthropic is None:
                    raise ProviderNotSupportedError(
                        "Anthropic SDK not installed. Install with: pip install anthropic"
                    )
                self.client = _RuntimeAsyncAnthropic(api_key=api_key, base_url=endpoint)
                logger.info(
                ...
                if _RuntimeAsyncInferenceClient is None:
                    raise ProviderNotSupportedError(
                ...
                logger.info(
                    f"Initialized HuggingFace client with model: {self.config.model}"
                )
    ...
    async def chat(self, messages: List[ChatMessage], **kwargs) -> str:
        ...
        try:
            ...
            raise AIServiceError(f"Chat completion failed: {str(error)}") from error
    ...
    async def _chat_anthropic(self, messages: List[ChatMessage], **kwargs) -> str:
        ...
        response = await client.messages.create(
            ...
            messages=cast(Any, conversation_messages),
            max_tokens=self.config.max_tokens,
            temperature=self.config.temperature,
            **kwargs,
        )
