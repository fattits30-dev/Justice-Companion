...
class AIProviderConfig(BaseModel):
    ...
    temperature: Optional[float] = Field(
        0.7, ge=0.0, le=2.0, description="Temperature (0.0-2.0)"
    )
    max_tokens: Optional[int] = Field(
        4096, ge=1, description="Maximum tokens to generate"
    )
    top_p: Optional[float] = Field(
        0.9, ge=0.0, le=1.0, description="Top-p sampling (0.0-1.0)"
    )
...
class UnifiedAIService:
    ...
    def _initialize_client(self) -> None:
        ...
        if provider == "ollama" and not api_key:
            ...
            raise HTTPException(
                status_code=400, detail=f"API key required for {provider}"
            )
        ...
        try:
            if provider == "anthropic":
                ...
                try:
                    ...
                    # AsyncInferenceClient uses base_url and api_key parameters
                    # Note: provider parameter only exists in sync InferenceClient
                    if self.config.endpoint:
                        # Custom endpoint (e.g., router.huggingface.co/v1)
                        self.client = InferenceClient(
                            base_url=self.config.endpoint, api_key=api_key
                        )
                    else:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to initialize {provider} client: {str(exc)}",
            )
    ...
    async def _stream_huggingface_chat(
        ...
        logger.info(f"[DEBUG] Stream created successfully, iterating...")
...
"""
            ...
            if not json_match:
                ...
                logger.warning(
                    f"[DEBUG] No JSON found in response. Response preview: {response[:500]}..."
                )
            ...
            try:
                ...
                logger.info(
                    f"[DEBUG] Successfully parsed case data: title={suggested_case_data.title}, type={suggested_case_data.case_type}"
                )
                logger.info(
                    f"[DEBUG] extracted_from data: {suggested_case_data.extracted_from}"
                )
                ...
                logger.error(
                    f"[DEBUG] JSON parse error: {exc}. JSON string: {json_str}"
                )
                ...
                logger.error(
                    f"[DEBUG] Error creating SuggestedCaseData: {exc}. Parsed JSON: {parsed_json}"
                )
                raise
