...
class LegislationResult(BaseModel):
    ...
    relevance: Optional[float] = Field(
        None, ge=0.0, le=1.0, description="0-1 relevance score"
    )
...
class ProcessQuestionInput(BaseModel):
    ...
    question: str = Field(
        ..., min_length=1, max_length=2000, description="User's legal question"
    )
    case_id: Optional[int] = Field(
        None, gt=0, description="Optional case ID for context"
    )
...
class AIResponse(BaseModel):
    ...
    code: Optional[str] = Field(
        None, description="Error code (NO_CONTEXT, SAFETY_VIOLATION, etc.)"
    )
...
class RAGStatistics(BaseModel):
    ...
    legislation_count: Optional[int] = Field(
        None, description="Number of legislation results"
    )
    case_law_count: Optional[int] = Field(
        None, description="Number of case law results"
    )
    knowledge_base_count: Optional[int] = Field(
        None, description="Number of knowledge base results"
    )
    total_context_size: Optional[int] = Field(
        None, description="Total context size in characters"
    )
...
class RAGService:
    ...
    async def process_question(
        ...
        try:
            ...
            logger.info(
                f"Question analyzed - keywords: {keywords}, category: {category}"
            )
            ...
            if not self._has_valid_context(context):
                logger.warning(
                    f"No legal context found for question: {question[:100]}..."
                )
            ...
            if ai_response.success:
                if not ai_response.message:
                    logger.error(
                        "AI response indicated success but returned no message body"
                    )
                    return AIResponse(
                        success=False,
                        message=None,
                        error="The AI response did not contain any content. Please try again.",
                        code="EMPTY_RESPONSE",
                ...
                if not validation_result.valid:
                    logger.error(
                        "AI response failed safety validation - violations: %s", validation_result.violations
                    )
            ...
            if self.audit_logger:
                self.audit_logger.log(
                    ...
                    details={
                        ...
                        "sources_count": (
                            len(ai_response.sources) if ai_response.success else 0
                        ),
                    },
                    success=True,
                )
            ...
            logger.exception("Error in RAGService.process_question: %s", error)
    ...
    def get_last_query_stats(self) -> RAGStatistics:
        ...
        if not self._last_query_stats:
            return RAGStatistics(
                ...
                legislation_count=None,
                case_law_count=None,
                knowledge_base_count=None,
                total_context_size=None,
            )
    ...
    async def _extract_and_analyze_question(self, question: str) -> List[str]:
        ...
        raw_keywords = keywords_dict.get("all")
    ...
    async def _fetch_legal_context(
        self, keywords: List[str], category: str
    ) -> LegalContext:
        """
        ...
        try:
            ...
            legislation_raw, case_law_raw, knowledge_base_raw = results

            legislation = self._normalize_context_results(
                legislation_raw, "legislation"
            )
            case_law = self._normalize_context_results(case_law_raw, "case law")
            knowledge_base = self._normalize_context_results(
                knowledge_base_raw, "knowledge base"
            )
            ...
            logger.error("Failed to fetch legal context: %s", error, exc_info=True)

            # Return empty context on API failure (don't crash)
            return LegalContext(legislation=[], case_law=[], knowledge_base=[])

    def _normalize_context_results(
        self, result: Any, label: str
    ) -> List[Dict[str, Any]]:
        """Convert API responses into a list of dictionaries expected by context limiters."""
        if isinstance(result, Exception):
            logger.error("%s API error: %s", label.capitalize(), result)
            return []

        if not isinstance(result, list):
            logger.warning(
                f"{label.capitalize()} API returned unexpected payload "
                f"of type {type(result).__name__}; ignoring result."
            )
            return []
        ...
        for entry in result:
            if isinstance(entry, dict):
                ...
                logger.warning(
                    f"{label.capitalize()} entry had unexpected type {type(entry).__name__} and was skipped."
                )
    ...
    def _limit_and_sort_legislation(
        ...
        for result in results:
            try:
                ...
                logger.warning("Failed to parse legislation result: %s", exc)
                continue
    ...
    def _limit_and_sort_case_law(
        ...
        for result in results:
            try:
                ...
                logger.warning(f"Failed to parse case law result: {exc}")
                continue
    ...
    def _limit_knowledge_base(
        ...
        for result in results:
            try:
                ...
                logger.warning(f"Failed to parse knowledge base result: {exc}")
                continue
...
def extract_sources(context: LegalContext) -> List[str]:
    ...
    for law in context.legislation:
        if law.url:
            source = (
                f"{law.title}{f' {law.section}' if law.section else ''} - {law.url}"
            )
...
SYSTEM_PROMPT_TEMPLATE = """You are a supportive UK legal information assistant for Justice Companion powered by Qwen 3.
Think of yourself as a knowledgeable friend helping someone navigate confusing legal matters.
Your role is to provide INFORMATION ONLY, never advice.
...
6. End EVERY response with this disclaimer:
...
✓ "That's a really important question - many people aren't sure about their rights here.
The Employment Rights Act 1996 states..."
...
IMPORTANT: You are NOT a solicitor. You do NOT provide legal advice. You do NOT guarantee outcomes.
You ONLY provide factual legal information to help UK citizens understand their rights -
but you do it with warmth and empathy."""
