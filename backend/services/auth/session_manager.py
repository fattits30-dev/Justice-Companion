...
class SessionManager:
    ...
    def __init__(
        self, db: Session, audit_logger=None, enable_memory_cache: bool = False
    ):
        """
        ...
        cache_state = "enabled" if enable_memory_cache else "disabled"
        logger.info(
            "SessionManager initialized (memory_cache=%s)",
            cache_state,
        )
    ...
    async def create_session(
        ...
        try:
            ...
            logger.info(
                "[SessionManager] Created session %s for user %s (expires: %s)",
                session_id,
                username,
            ...
            raise SessionManagerError(f"Failed to create session: {str(exc)}")
    ...
    async def validate_session(self, session_id: str) -> SessionValidationResult:
        ...
        if self.enable_memory_cache and session_id in self._memory_cache:
            ...
            if datetime.now(timezone.utc) > cached_session.expires_at:
                ...
                try:
                    ...
                    logger.error(
                        "Failed to delete expired session %s: %s", session_id, exc
                    )

                logger.info(
                    "[SessionManager] Cleaned up expired cached session %s for user %s",
                    session_id,
        ...
        try:
            ...
            if db_session.expires_at.replace(tzinfo=timezone.utc) < now:
                ...
                logger.info(
                    "[SessionManager] Cleaned up expired session %s for user %s",
                    session_id,
                    user_id,
                )
            ...
            if self.enable_memory_cache and session_id not in self._memory_cache:
                memory_session = InMemorySession(
                    ...
                    remember_me=(db_session.expires_at - db_session.created_at).days
                    > 1,
                )
                self._memory_cache[session_id] = memory_session
    ...
    async def destroy_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            if db_session:
                ...
                logger.info(
                    "[SessionManager] Destroyed session %s for user %s",
                    session_id,
                    username,
                )
            ...
            logger.error("Failed to destroy session %s: %s", session_id, exc)
            return False
    ...
    async def cleanup_expired_sessions(self) -> int:
        ...
        try:
            ...
            if db_cleaned > 0:
                ...
                logger.info(
                    "[SessionManager] Cleaned up %d expired sessions (cache: %d, db: %d)",
                    cleaned_count,
                    len(expired_cache_ids) if self.enable_memory_cache else 0,
                    db_cleaned,
                )
    ...
    def get_session_count(self) -> Dict[str, int]:
        ...
        try:
            ...
            logger.error("Failed to get session counts: %s", exc)
    ...
    def get_active_session_ids(self) -> List[str]:
        ...
        try:
            ...
            logger.error("Failed to get active session IDs: %s", exc)
            return []
    ...
    async def get_user_sessions(self, user_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error("Failed to get user sessions for user %s: %s", user_id, exc)
            return []
    ...
    async def revoke_user_sessions(
        ...
        try:
            ...
            logger.info(
                "[SessionManager] Revoked %d sessions for user %s (except: %s)",
                revoked_count,
                user_id,
                except_session_id or "none",
            )
