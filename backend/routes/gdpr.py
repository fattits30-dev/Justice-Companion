...
def decrypt_field(
    ...
    try:
        ...
        if all(
            key in encrypted_data_dict
            for key in ["algorithm", "ciphertext", "iv", "authTag", "version"]
        ):
...
def export_user_data(
    ...
    # Export user profile
    sql = text(
        "SELECT id, username, email, created_at, updated_at, last_login_at FROM users WHERE id = :user_id"
    )
    ...
    return {
        ...
        "userData": {
            ...
            "chatConversations": {
                ...
                "records": chat_conversations,
                "count": len(chat_conversations),
            },
            "chatMessages": {
...
def delete_user_data(db: Session, user_id: int, reason: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        result = db.execute(
            text("DELETE FROM chat_conversations WHERE user_id = :user_id"), {"user_id": user_id}
        )
        ...
        # 4. Cases
        result = db.execute(
            text("DELETE FROM cases WHERE user_id = :user_id"), {"user_id": user_id}
        )
        deleted_counts["cases"] = result.rowcount

        # 5. User facts
        result = db.execute(
            text("DELETE FROM user_facts WHERE user_id = :user_id"), {"user_id": user_id}
        )
        ...
        result = db.execute(
            text("SELECT COUNT(*) FROM consents WHERE user_id = :user_id"), {"user_id": user_id}
        )
...
async def gdpr_export(
    ...
    try:
        ...
        logger.info(
            f"GDPR export complete: {export_data['metadata']['totalRecords']} records exported"
        )
...
async def gdpr_delete(
    ...
    try:
        ...
        logger.warning(
            f"GDPR deletion complete: {sum(deletion_result['deletedCounts'].values())} records deleted"
        )
...
async def update_consent(
    ...
    try:
        ...
        if request.granted:
            ...
            db.execute(
                sql, {"user_id": user_id, "consent_type": request.consentType, "revoked_at": now}
            )
        ...
        log_audit_event(
            ...
            action="update",
            details={"consentType": request.consentType, "granted": request.granted},
            success=True,
        )
