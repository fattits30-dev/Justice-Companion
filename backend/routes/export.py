...
from backend.services.export.export_service import ExportService, ExportOptions as ServiceExportOptions
...
class ExportOptions(BaseModel):
    ...
    template: Optional[Literal["case-summary", "evidence-list", "timeline-report", "case-notes"]] = Field(
        None,
...
class ExportTemplate(BaseModel):
    ...
    formats: List[Literal["pdf", "docx", "json", "csv"]] = Field(..., description="Supported formats")
...
async def export_evidence_to_pdf(
    ...
    **Raises:**
    ...
    try:
        ...
        result = await export_service.export_evidence_list_to_pdf(
            case_id=case_id,
            user_id=user_id
        )
        ...
        logger.error(f"Failed to export evidence to PDF for case {case_id}: {str(e)}", exc_info=True)
...
async def export_timeline_to_pdf(
    ...
    **Raises:**
    ...
    try:
        ...
        logger.error(f"Failed to export timeline to PDF for case {case_id}: {str(e)}", exc_info=True)
...
async def export_notes_to_pdf(
    ...
    **Raises:**
    ...
    try:
        ...
        # Call ExportService convenience method
        result = await export_service.export_case_notes_to_pdf(
            case_id=case_id,
            user_id=user_id
        )
...
async def export_notes_to_docx(
    ...
    **Raises:**
    ...
    try:
        ...
        # Call ExportService convenience method
        result = await export_service.export_case_notes_to_word(
            case_id=case_id,
            user_id=user_id
        )
    ...
    try:
        ...
        api_templates = [
            ExportTemplate(
                ...
                formats=["pdf", "docx"] if template.name != "Case Summary" else ["pdf", "docx", "json", "csv"]
            )
