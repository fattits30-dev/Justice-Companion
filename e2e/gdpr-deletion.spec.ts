...
// Test database path
const TEST_DB_PATH = path.join(process.cwd(), '.test-e2e-gdpr', 'justice-test.db');
...
test.describe.serial('GDPR Article 17 - Right to Erasure', () => {
  ...
  test('Step 1: Register a new user', async () => {
    ...
    const response = await window.evaluate(
      async ({ username, email, password }) => {
        return await (window as any).justiceAPI.register(username, email, password);
      },
    ...
    console.log('[E2E GDPR] User registered successfully:', { userId, sessionId });
  });
  ...
  test('Step 3: Create test data (cases, evidence, chat messages)', async () => {
    ...
    const cases = db.prepare('SELECT * FROM cases WHERE userId = ?').all(userId);
    const evidence = db
      .prepare('SELECT * FROM evidence WHERE caseId IN (SELECT id FROM cases WHERE userId = ?)')
      .all(userId);
  ...
  test('Step 4: Export user data (GDPR Article 20)', async () => {
    ...
    const exportResponse = await window.evaluate(
      async ({ sessionId }) => {
        return await (window as any).electronAPI.gdpr.exportData(sessionId, { format: 'json' });
      },
      { sessionId }
    );
  ...
  test('Step 6: Verify data is deleted from database', async () => {
    ...
    const consents = db.prepare('SELECT * FROM consent_records WHERE userId = ?').all(userId);
    expect(consents.length).toBe(2);
  ...
  test('Step 7: Verify user cannot login after deletion', async () => {
    console.log('[E2E GDPR] Attempting to login as deleted user:', testUsername);

    const loginResponse = await window.evaluate(
      async ({ username, password }) => {
        return await (window as any).justiceAPI.login(username, password, false);
      },
...
test.describe.serial('GDPR Article 17 - Error Scenarios', () => {
  ...
  test.beforeAll(async () => {
    ...
    if (!electronApp) {
      throw new Error('Electron app not initialized. Run main test suite first.');
    }
  });
