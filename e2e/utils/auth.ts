...
async function fillFirstAvailable(
  locatorFactories: LocatorFactory[],
  page: Page,
  value: string,
  errorMessage: string
) {
  ...
  try {
    ...
    await page.evaluate(() => {
      try {
        ...
        console.debug("Storage access denied, continuing without clearing", (e as Error).message);
      }
    });
  } catch (error) {
...
export async function loginWithSeededUser(page: Page) {
  ...
  await fillFirstAvailable(
    ...
    page,
    TEST_USER.username,
    "Unable to locate username/email field on login form"
  );
  ...
  await fillFirstAvailable(
    ...
    page,
    TEST_USER.password,
    "Unable to locate password field on login form"
  );
