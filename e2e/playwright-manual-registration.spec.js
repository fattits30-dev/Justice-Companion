...
async function loginNewlyRegisteredUser(page, email, password) {
...
test.describe("Manual Registration Flow", () => {
    test("should successfully register a new user and navigate to dashboard", async ({ page, }) => {
        ...
        await expect(page.getByRole("heading", { name: /welcome to justice companion/i })).toBeVisible();
        // Step 9: Verify user is logged in (check for user profile button)
        await expect(page.getByRole("button", { name: /open profile manager/i })).toBeVisible();
        ...
        await expect(page.getByRole("button", { name: /upload evidence/i })).toBeVisible();
