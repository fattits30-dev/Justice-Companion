...
const TEST_DOCUMENT_PATH = path.join(__dirname, "..", TEST_CONFIG.testDocuments.dismissalLetter);
test.describe("Document Upload and Analysis Flow", () => {
    ...
    test("should complete full document upload and analysis flow", async () => {
        ...
        try {
            ...
            await page
                .waitForSelector('text=/upload.*success|file.*uploaded/i, [class*="success"], .text-green-500', {
                timeout: 10000,
                state: "visible",
            })
                .catch(async (e) => {
        ...
        try {
            ...
            const errorElements = await page
                .locator('[role="alert"].error, .text-red-500:has-text("error"), .text-destructive:has-text("error")')
                .count();
