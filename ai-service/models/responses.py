...
class ConfidenceScores(BaseModel):
    ...
    nextHearingDate: float = Field(..., ge=0.0, le=1.0, description="Confidence in next hearing date")
...
class SuggestedCaseData(BaseModel):
    ...
    documentClaimantName: Optional[str] = Field(
        None,
        description="Claimant name from document (if different from user)"
    )
    ...
    extractedFrom: ExtractedFields = Field(..., description="Source information for extracted fields")
...
class DocumentAnalysisResponse(BaseModel):
    ...
    metadata: Optional[Dict[str, Any]] = Field(
        None,
        description="Optional metadata (tokens used, latency, etc.)"
    )
