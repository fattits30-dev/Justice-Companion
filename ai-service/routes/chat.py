...
class ChatRequest(BaseModel):
    ...
    model_preference: Optional[str] = Field(
        default="balanced", description="fast|balanced|thorough"
    )
...
async def chat_completion(request: ChatRequest, req: Request):
    ...
    # Check if this is a complex request that needs the orchestrator
    # For now, we'll use a simple heuristic or just use it for "thorough" requests
    # Or we can just use it for everything if it's fast enough.
    # Let's use it for "thorough" or if the prompt implies multiple steps.

    use_orchestrator = (
        request.model_preference == "thorough"
        or "draft" in request.messages[-1].content.lower()
        or "research" in request.messages[-1].content.lower()
    )
    ...
    try:
        if use_orchestrator:
            ...
            tools = get_agent_tools(hf_client)
            orchestrator = OrchestratorAgent(hf_client, tools)

            # Combine messages into a single request string for the orchestrator
            # (Orchestrator currently takes a string request)
            last_message = request.messages[-1].content

            # If there's context from previous messages, we should probably include it
            # For now, just sending the last user message

            agent_response = await orchestrator.process(last_message)

            return ChatResponse(
                content=agent_response.content,
                model_used="orchestrator",
                tokens_used=0,  # TODO: Track tokens
                sources=None,  # TODO: Extract sources from agent response
            )
        else:
            # Standard chat
...
async def analyze_case(
    case_description: str, case_type: Optional[str] = None, req: Request = None
):
    """
