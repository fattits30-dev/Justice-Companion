...
export function setupCaseHandlers(): void {
  ...
  ipcMain.handle(
    'case:create',
    async (_event: IpcMainInvokeEvent, data: unknown, sessionId: string, aiMetadata?: unknown): Promise<IPCResponse> => {
  ...
  ipcMain.handle('case:list', async (_event: IpcMainInvokeEvent, sessionId: string): Promise<IPCResponse> => {
    return withAuthorization(sessionId, async (userId) => {
      try {
        ...
        console.warn('[IPC] Retrieved', (userCases as unknown[]).length, 'cases for user:', userId);
    ...
    async (_event: IpcMainInvokeEvent, id: unknown, sessionId: string): Promise<IPCResponse> => {
      return withAuthorization(sessionId, async (userId) => {
        try {
          ...
          const validatedData = caseSchemas.caseGetByIdSchema.parse({ id: numericId });
          ...
          console.warn('[IPC] Case retrieved:', (caseData as { id: number }).id);
    ...
    async (_event: IpcMainInvokeEvent, id: unknown, data: unknown, sessionId: string): Promise<IPCResponse> => {
      return withAuthorization(sessionId, async (userId) => {
        try {
          ...
          const validatedData = caseSchemas.caseUpdateSchema.parse({ id: numericId, input: data });
    ...
    async (_event: IpcMainInvokeEvent, id: unknown, sessionId: string): Promise<IPCResponse> => {
      return withAuthorization(sessionId, async (userId) => {
        try {
          ...
          // Validate ID with Zod
          const validatedData = caseSchemas.caseDeleteSchema.parse({ id: numericId });
          ...
          if (!existingCase) {
            console.error('[IPC] case:delete - Case not found or unauthorized:', validatedData.id, 'userId:', userId);
            throw new CaseNotFoundError(validatedData.id);
          }

          console.warn('[IPC] case:delete - Found case, attempting deletion...');
          ...
          console.warn('[IPC] case:delete - DELETE result:', { changes: result.changes, lastInsertRowid: result.lastInsertRowid });

          if (result.changes === 0) {
            console.error('[IPC] case:delete - No rows deleted for case:', validatedData.id);
            throw new CaseNotFoundError(validatedData.id);
          }
          ...
          console.warn('[IPC] Case deleted successfully:', validatedData.id, 'rows affected:', result.changes);
          return successResponse({ deleted: true, id: validatedData.id });
        } catch (error) {
          console.error('[IPC] case:delete error:', error);
          console.error('[IPC] case:delete error stack:', error instanceof Error ? error.stack : 'No stack trace');
