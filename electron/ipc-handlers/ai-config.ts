...
export function setupAIConfigHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        console.warn('[IPC] ai:configure called', { provider: request.provider, model: request.model });
        ...
        await configService.setProviderConfig(request.provider, request.apiKey, {
          model: request.model,
        ...
        console.warn('[IPC] AI provider configured successfully:', request.provider);
        ...
        return {
          success: false,
          error: error instanceof Error ? error.message : 'Failed to configure AI provider',
        };
      }
    }
  );
  ...
  ipcMain.handle(
    ...
    async (
      event: IpcMainInvokeEvent,
      request: { provider: AIProviderType }
    ): Promise<IPCResponse> => {
      try {
        console.warn('[IPC] ai:test-connection called for provider:', request.provider);
